...
#       confidence = 0.0
#     end
#     Keyword.create(text: keyword["text"], sentiment: sentiment, confidence: confidence, analyzed_post_id: new_post.id)
#   end

# end

CSV.open('db/analyzed_posts.csv', 'wb') do |csv|
  csv << AnalyzedPost.attribute_names
  AnalyzedPost.all.each do |post|
    csv << post.attributes.values
  end
end

CSV.open('db/keywords.csv', 'wb') do |csv|
  csv << Keyword.attribute_names
